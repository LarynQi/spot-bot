...
# DB_NAME = db_client.get_database(DATABASES['180-fa23']).get_collection('curr_db').find({})
...
@bolt_app.message("sp23-Scoreboard")
def sp23_scoreboard(event, say):
    scoreboard(event, say, prefix="sp23-", db_name=DATABASES['180'])
...
@bolt_app.message("sp23-Caughtboard")
def sp23_caughtboard(event, say):
    caughtboard(event, say, prefix="sp23-", db_name=DATABASES['180'])
...
def sp23_pics(event, say):
    pics(event, say, db_name=DATABASES['180'])
...
def _reset():
    ...
    new_DB_NAME = init_db(db_client, f'180|{dt}')
    return new_DB_NAME
